...
namespace gb.TimeSheets.Repos
{
    public class FakeRepository : IFakeRepository
    {
        ...
        public IEnumerable<Person> GetFromTo(int skip, int take)
        {
            return data.GetRange(skip, take);
        }

        public void Update(Person person)
        {
            var updatedPerson = data.FirstOrDefault((x) =>
            {
            ...
            {
                foreach (var item in data.Where(x => x.Id == person.Id))
                {
                    item.Age = person.Age;
            ...
            {
                throw new System.Exception("Пользователь для обновления не найден");
            }
